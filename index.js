...
async function handlePrepare(interaction, seedType, presetValue) {
  ...
  try {
    ...
    const doneMsg = isRandomPrep
      ...
      : isFullyRandomPrep
      ? `Preparation complete. I was tasked to choose an entirely random preset. The preset I chose is: **${job.resolvedPresetName}** (${seedTypeLabelPrep})\nSeed-Hash: ${job.seedHash}. The generation took ${formatDuration(job.durationMs)}.`
...
async function deliverPrepared(interaction, preparedJob) {
  ...
  const rolledLine = isRandom
    ...
    : isFullyRandom
    ? `I was tasked to pick an entirely random preset. The preset I chose is: **${preparedJob.resolvedPresetName}** (${seedTypeLabel})`
...
async function handleInfo(interaction) {
  const embed = new EmbedBuilder()
    ...
    .addFields(
      { name: 'How it works', value: 'OoTMM Randomizer seeds are rolled based on your selected seed type (Solo, Solo Multiworld (3 Players), Duo Multiworld (2 Players), Race Multiworld (2 Teams of 3 players)) and preset. If the queue is full, please wait and try again. Random presets pick one of the available variant options.' },
...
async function handleGenerate(interaction, seedType, presetValue) {
  ...
  try {
    ...
    const rolledLine2 = random
      ...
      : isFullyRandom
      ? `I was tasked to pick an entirely random preset. The preset I chose is: **${job.resolvedPresetName}** (${seedTypeLabel2})`
